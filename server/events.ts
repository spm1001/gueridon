...
export type BridgeEvent =
  ...
  | { type: "turn:complete"; folder: string; sessionId: string; durationMs: number; inputTokens: number | null; outputTokens: number | null; contextPct: number | null; toolCalls: number; subagentFiltered?: number; sseBytes?: Record<string, number>; sseSkippedBytes?: number }
  ...
  | { type: "session:auto-resume-fail"; folder: string; error: string }
  ...
  // Remote control — Future B launcher (gdn-difoto, gdn-senila)
  | { type: "rc:spawn"; folder: string; pid: number }
  | { type: "rc:exit"; folder: string; pid: number; exitCode: number }
  | { type: "rc:url"; folder: string; pid: number; url: string }
...
const LEVEL_MAP: Record<BridgeEvent["type"], LogLevel> = {
  ...
  "session:auto-resume-fail": "error",
  "handoff:stale": "info",
  "replay:ok": "info",
  "replay:fail": "warn",
  "push:init": "info",
