...
Pickup flow: tap a RECENT row → wallet sheet → the conversation page force-resumes that uuid under that spawn env, forking when the wallet differs from last-observed. **The wallet sheet is an open, config-driven list, not two hardcoded buttons** (Sameer, 2026-08-29): the two-Teams-seats theory (background vs invocation) didn't survive contact, a third Teams seat is incoming, and the actual requirement is "flip any session to any billing". Each lane is a name plus an env block (`CLAUDE_CONFIG_DIR=…` for seats, the Vertex var set for ITV). This works today for all three wallets because billing is just the spawn env on the `-p` lane (the streaming stack is wallet-agnostic plumbing). **Pickup into the claude.ai surface: MEASURED WORKING, 2026-08-30 (gdn-vutogu).** `claude --remote-control <name> --resume <uuid> --fork-session` on a cold session came up with the history replayed, `/remote-control` active, a fresh `claude.ai/code/session_…` URL, Teams billing, and the original transcript byte-identical (fork). So a cold session CAN be sent to the phone-native surface in one command — on the account whose login will view it (web bookmark per wallet). No new turn runs at spawn, so the send itself costs no quota. Fallback route also documented upstream: `claude --resume <uuid>` in a pty, then the `/remote-control` slash command.
...
**Which-launcher-when (the confusion clause):** sharpened by the wallet reframe (2026-08-29). The iOS app is **locked to one login** — you can't install two copies — but two updates from Sameer's own measurements (2026-08-30): **tiles confirmed to follow the server's login id** (a `claude rc` server running under the commis config dir surfaced its tile under the family account), and **the claude.ai web app dissolves the one-app lock** — he now drives via web bookmarks, one labelled bookmark per account, plus the Guéridon bookmark. Consequence: RC "which account sees this tile" = "which config dir runs the server", and multi-account phone driving is web-bookmark-shaped, not app-shaped. Roster remote rows carry the session's own claude.ai URL as Open (cse_↔session_ one-id derivation), which drops straight into that flow. So the rule becomes: *iOS app tiles = quick new session on the app's own account, in a hot repo; Guéridon = every launch or pickup that involves choosing a wallet* — which, with three-plus Teams seats and Vertex, is most of them. Guéridon is structurally the only multi-wallet launcher. gdn-sudacu's question shifts accordingly: not "retire the Teams button" but "does the RC spawn grow a seat picker" (noting an off-app-account attach URL opens only in Safari under that account's login, not in the app).
