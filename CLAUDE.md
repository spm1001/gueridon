...
- **Teams lane** — `claude --remote-control` in a pty; the phone gets a `claude.ai/code/session_…` link and claude.ai's native UI does the driving. Billed to **Teams/MAX**. Gated on `GUERIDON_ENABLE_RC=1` (LIVE in prod since 2026-06-29). **⚠ UNDER REVIEW for retirement since 2026-07-25 — read the substrate note below before building anything on it.**

> **Substrate note, 2026-07-25 — remote launch now exists natively, and the Teams lane is a duplicate.**
> `claude remote-control` (alias `claude remote`) has a **server mode**: a persistent supervisor
> that registers a *directory* with claude.ai, after which the Claude mobile app creates sessions
> in it **on demand** (up to `--capacity`, default 32). Verified live on tube: it runs headless
> under systemd (`claude-remote@home-modha-notes` is live, ~106 MB idle), **one device shows N
> directories** (one tile per server), and the whole phone→desk loop works natively — create from
> the phone, Archive from the phone (clean end, no orphan), `claude --resume` at the desk with
> history intact. So the app now provides launch, drive, roster, push and file-attachments better
> than the Teams lane does. **The Vertex lane is unaffected and is now provably unique:** Remote
> Control hard-refuses when `CLAUDE_CODE_USE_VERTEX` is set (matched-pair verified — the gate is
...
Phone URL: `https://<your-tailscale-hostname>/` (Tailscale HTTPS termination). Set `TAILSCALE_HOSTNAME` env var. Bare `/` redirects to the launcher (`launch.html`); a deep-link `/#<owner/repo>` (raw, slash intact) opens that repo's Vertex/streaming session directly — *resuming* the folder's latest session. The launcher's **Vertex** button instead starts a **fresh** session (gdn-duhino): it sets a one-shot `sessionStorage` flag `gdnFreshLaunch` that the conversation page consumes on first connect to send `sessionId:"new"`, so a new launch never silently resumes the last session. The roster's **Open** (a bare `/#folder`) is the resume path.
...
- **Two `EnvironmentFile`s.** `/opt/gueridon/.env` holds `TAILSCALE_HOSTNAME`, `VAPID_SUBJECT`, `ENABLE_CLAUDEAI_MCP_SERVERS`, `CC_MODEL`, and optionally `EXTRA_FOLDERS` (gdn-seroba — colon-separated absolute paths listed in the launcher besides the scan tree; never recursed into, creation stays under `SCAN_ROOT`) (not in the unit file; `.env.example` has placeholders; `.env` is gitignored). The unit **also** sources `/home/<user>/.dotfiles/shell/gueridon.env` (was `/etc/claude-code/vertex.env` until 2026-08-02) — the dotfiles Vertex config (`CLAUDE_CODE_USE_VERTEX=1`, `ANTHROPIC_VERTEX_PROJECT_ID`, model IDs) — which is what puts the **Vertex lane** (`claude -p`) on Vertex billing. The **Teams lane** spawns strip this set per-spawn (`buildRemoteControlEnv`) so RC comes up on Teams, which is the only billing the claude.ai relay attaches to.
...
- **Session ledger** — `~/.config/gueridon/session-ledger.jsonl` (gdn-daluto), append-only, written by the bridge from CC's session registry while the roster is enabled. Lives under Guéridon's state dir rather than either seat's config dir because it spans both seats; on the atelier each person's bridge has its own `HOME`, so it is per-person by construction. Safe to delete (history lost, nothing breaks); never hand-edit while a bridge runs.
...
| `sessions.ts` | **Launcher:** `/proc`-scan for live `claude` sessions — by `comm=="claude"` OR exe under `~/.local/share/claude/versions/` (gdn-zahidu: comm alone missed RC-server children, whose comm is the version string) — feeds the launcher's `GET /sessions` roster (gdn-batogo). Also derives wallet (`CLAUDE_CONFIG_DIR`/Vertex markers) and, for RC children, the `cse_` id → local transcript uuid (`teleportSessionUuid`) |
| `registry-watch.ts` | **Launcher:** inotify (`fs.watch`) watcher on CC's session registry — `~/.claude/sessions/<pid>.json` and `~/.claude-commis/sessions/` (gdn-fusijo). Each record carries `status` (`idle`/`busy`/`waiting`/`shell`…), `statusUpdatedAt`, `tmux` pane, `sessionId`, `bridgeSessionId`; the directory names the seat. `classifyStatus` maps onto the roster's four-way `state` — `waiting` and `busy` by name, anything else CC writes → `idle` ("fine"), ABSENT → `unknown` (batch `-p` sessions write no status; phone `--sdk-url` children DO carry one on 2.1.270, with `tmux` null — the discriminator is `-p` vs `--sdk-url`, not the `sdk-cli` entrypoint; never idle). Merged on pid in `buildSessionRoster` via `byPid()`; `snapshot()` feeds `/recent`'s liveness union. Plus a 10 s reconcile pass (missed inotify events, a seat directory that appears later). Only `<pid>.json` is ever opened — the sibling `<pid>.<hash>.key` files are messaging secrets. Started in the main-guard when the roster is enabled; emits `registry:watch` per directory (`armed`/`missing`/`error`). Two sinks by design: live state now, and the append-only ledger (gdn-daluto) subscribes to its `upsert`/`remove` events — never build a second reader |
| `session-ledger.ts` | **Launcher:** the watcher's second sink (gdn-daluto) — an append-only JSONL at `~/.config/gueridon/session-ledger.jsonl` keeping the join CC's registry deletes at exit: one row per (`sessionId`, seat directory) — never keyed on pid — with `seat` (from the directory), `wallet` (the Vertex-vs-Teams tag read from `/proc` once at join time via `vertexBilledForPid`; null when unreadable, never a guess), `bridgeSessionId` (the `session_…` phone-app id), cwd, entrypoint, kind, name, tmux pane, `first_seen`/`last_seen`/`ended_at`/`ended_reason`. Subscribes to `RegistryWatcher`'s `upsert`/`remove`; boot backfill from `snapshot()` (records present → rows, open rows whose record is gone → `absent-at-boot`). A line lands on open / reopen / identity change / end — status flips move `last_seen` in memory only. `lookup(q)` answers the three questions (seat for a uuid, uuid for a `session_…`/`cse_…` id, live holders) and flags a uuid5 mismatch. CLI with no bridge: `npx tsx server/session-ledger.ts <uuid \| session_… \| cse_…>` (its `liveNow` probes the record file directly). Emits `ledger:start` (fold counts) and `ledger:problem` (torn tail on load, failed append) |
...
| GET | `/sessions` | **Launcher (gated):** the launcher roster — EVERY live `claude` session (gdn-batogo), not just RC ones. `{sessions:[{pid,name,cwd,ageSec,kind,attachable,url,ready,wallet,state,stateSince,sessionUuid?,tmux?}]}`, newest first. `wallet` (gdn-zahidu) is display-ready ("vertex" / "sameer@" / "family@" / config-dir basename — `walletLabel` in bridge-logic). `state` (gdn-fusijo) is `idle`/`busy`/`waiting`/`unknown` from CC's session registry (`registry-watch.ts`), `stateSince` its ms-epoch `statusUpdatedAt` (null when unknown), `tmux` the pane id when the registry names one; `waiting` = a permission / AskUserQuestion / plan dialog is up and the session cannot progress without a human — measured to reach this endpoint 205–276 ms after CC stamps it. `unknown` means the registry has no status for the pid (batch `-p` sessions; phone `--sdk-url` children do carry one) and is shown as such, never as idle. Five kinds (gdn-riheri, +gdn-kuhaku, +gdn-zahidu): `kind:"rc"` = Guéridon-spawned RC (attachable; Open = claude.ai URL, End = `DELETE /launch`); `kind:"vertex"` = Guéridon's own streaming-lane `-p` session (attachable; Open = RAW `/#folder` — never percent-encoded, End = `POST /exit`); `kind:"vertex-terminal"` = a Vertex-billed session Guéridon did NOT launch (a `claudev`/`claudefv` terminal session — detected from `/proc` environ OR the cmdline `--settings` blob; shown "terminal", wallet chip carries the vertex); `kind:"remote"` = a phone-created session child under a `claude-remote@`/`claude rc` server (gdn-zahidu — identified by its `--sdk-url …/cse_…` cmdline; carries `sessionUuid` derived via `teleportSessionUuid`; shown "phone"; NOT attachable, and deliberately **NO End** until SIGTERM-on-a-server-child is measured — archive-from-phone is the known-clean end); `kind:"local"` = hand-started/foreign non-Vertex (a Claude Desktop-app agent, a terminal session in `~`, a daemon background fork, or an RC session orphaned by a bridge restart). `vertex-terminal` and `local` are **End-able but NOT attachable** (End = `DELETE /session/:pid`, gdn-racuca — SIGTERM by pid, graceful/resumable, no pty needed; no Open — takeover is gdn-merozu). `/proc`-scans by `comm=="claude"` OR exe under `~/.local/share/claude/versions/` (gdn-zahidu — comm alone MISSED RC-server children, whose comm is the version string, e.g. "2.1.251"), cross-refs `rcSessions` + the `-p` `sessions` map by pid; own-vertex wins over the generic vertex-billed detection. **Claude INFRA processes are excluded** (`isInfraCmdline` in `sessions.ts`, subcommand ∈ {`daemon`, `remote-control`, `remote`, `rc`, `agents`, `bg-spare`, `bg-pty-host`} — read from argv[1] OR argv[0]'s second word, because some infra rewrites its title folding the subcommand into argv[0] as one space-joined token, measured 2026-08-30): the CC daemon (gdn-mimije), RC **servers** (gdn-caguga — an End-able server row would also permanently hide its repo from the launch list via gdn-wuvujo; `rc` is a second server alias seen live 2026-08-30 under the commis wallet), the `claude agents` TUI, and the daemon's bg-spare/bg-pty-host pool (gdn-zahidu). The Teams lane's `--remote-control` FLAG form (dashes) is a real session and is NOT excluded. |
...
| GET | `/recent` | **Launcher (gated like `/sessions`):** the uuid-keyed layer (gdn-vucube) — recent COLD sessions from the transcript farm, cold-by-construction (excludes every uuid the liveness union knows: /proc scan sessionUuids + Guéridon's own registries + per-wallet `claude agents --json` + the watched session registry, counting a registry record only when its pid is alive — a stale record (gdn-daluto, 2026-09-14) used to hide exactly the dead phone sessions this band exists to show; `claude agents --json` already filters dead pids itself, measured the same night). `{sessions:[{uuid,name,cwd,ageSec,title,titleSource,firstPrompt,entrypoint,uuidVersion,seat?,wallet?,seats?,teleportId?,lastSeen?,endedAt?}]}` — titles scavenged ai-title → Cowork sidecar → RC bridge-log scrape → first prompt (`server/session-index.ts`, deglacer-schema-informed). The optional tail (gdn-daluto) comes from the session ledger's newest row for the uuid — `seat`/`wallet` = which lane ran it at last sighting (`wallet` null when Vertex-or-not was unreadable), `seats` = every seat it ran on, `teleportId` = its `session_…` phone-app id — and is absent when the ledger never saw the session. 5-day window, cap 15, 20s response cache. Read-only v1 — pickup verbs land with gdn-merozu |
| GET | `/ledger?session=` | **Launcher (gated like `/sessions`):** the session ledger's read helper (gdn-daluto). `session` is a transcript uuid, a `session_…` or a `cse_…` id; 400 on anything else. Returns `{query,kind,uuid,teleport,rows,live,held,uuidMismatch}` — `rows` = every ledger row for the session (one per seat, newest first, each with `seat`, `wallet`, `pids`, `cwd`, `tmuxPane`, `first_seen`, `last_seen`, `ended_at`, `ended_reason`), `live` = the rows whose registry record is still present, each with `pidAlive`, and `held` = some such pid is actually alive. Only `held` means a driver exists (SIGTERM it before any plain `--resume`, else two writers on one JSONL): **a record can outlive its process** — measured 2026-09-14, two phone-child records with pids dead ~2 h, one still `busy` — so `live` non-empty with `held:false` is a stale record, not a session to wait for (`pidAlive` also requires the pid to be a Claude binary, so a recycled pid does not count). `uuid` = as given, else the sessionId OBSERVED on a row carrying the teleport id, else the uuid5 derivation (`uuidSource` says which; `derivedUuid` is always reported on a teleport query). **Use `uuid`, never `derivedUuid`, to build a resume command:** an ordinary terminal session on a Teams seat acquires a `bridgeSessionId` too (remote control on a `cli` session) and its transcript is its own v4 uuid, so the derivation names a transcript that does not exist. `uuidMismatch` fires only for a v5 sessionId that disagrees with the derivation — the tripwire for CC's client constants moving |
| DELETE | `/session/:pid` | **Launcher (gated; numeric-only match, no collision with `POST /session/:folder`):** end ANY live `claude` session by pid — incl. FOREIGN ones the roster lists but Guéridon doesn't own (gdn-racuca). `handleSessionEnd` verifies `/proc/<pid>/comm=="claude"` (`isLiveClaudePid`, `sessions.ts` — rules out a recycled/foreign pid; 404 otherwise) then `process.kill(pid,"SIGTERM")`, SIGKILL fallback @ 8s re-checking identity. No pty handle needed — the SIGTERM-by-pid primitive, the End-only slice of the parked gdn-kidowe. *(Built + pushed @018d7e8; live on next deploy.)* |
...
hand-started/foreign sessions are End-able-not-attachable (End = `DELETE /session/:pid`,
gdn-racuca) — a Vertex-billed one Guéridon didn't launch shows as
...
| `handleSessionEnd(pid)` + `isLiveClaudePid(pid)` | `bridge.ts` / `sessions.ts` | End a FOREIGN session by pid (gdn-racuca): `isLiveClaudePid` gates on `/proc/<pid>/comm=="claude"` (fail-closed → 404), then SIGTERM by pid, SIGKILL fallback @ 8s re-checking identity. Feeds `DELETE /session/:pid`. Unit-tested (real `claude`-comm child SIGTERM'd dead) + HTTP integration (gated-404 / non-claude-404 / real-process end-to-end). The `endSession` client branch routes foreign kinds here (owned rc/vertex keep their folder-keyed ends). |
...
- **Folder trust:** *no seeding needed.* CC trust **cascades from the nearest trusted ancestor dir** (empirically verified); `~/repos` is trusted and `SCAN_ROOT=~/repos`, so every launch inherits trust. Precondition for a fresh machine: trust `~/repos` once (a rebuild/setup step, not gueridon's job). **`EXTRA_FOLDERS` entries sit outside that cascade** — each must be trusted itself before its first launch (`~/notes` already is).
...
- No in-conversation session switcher (retired in gdn-deloce). The launcher (`launch.html`) is the home/chooser AND the session switchboard (gdn-vagori): repo list by git-recency + a live roster of every `claude` session — Guéridon-owned entries (rc/vertex) are tappable (Open/End), foreign ones End-able-not-attachable (End by pid — `DELETE /session/:pid`, gdn-racuca, with a two-tap "Sure?" arm; a foreign Vertex session shows "terminal" with a vertex wallet chip, gdn-kuhaku; every row carries a wallet chip and phone-created RC children show as "phone" with an Open link to their claude.ai page, gdn-zahidu; every row also carries a STATE chip — idle / busy / waiting / unknown with age, from CC's session registry via the bridge's inotify watcher, `waiting` filled amber because it is the one the phone must never miss, gdn-fusijo); the roster re-fetches on tab wake and polls every 20s while visible. Below it, a RECENT band (gdn-vucube, `GET /recent`) lists cold sessions from the farm — scavenged titles, first-prompt subtitles, 100KB substance floor, search-filterable — read-only until pickup (gdn-merozu); each row also carries a seat chip and, for a phone conversation, its `session_…` id, both from the session ledger (gdn-daluto) since the transcript cannot say either (dashed chip = seat known, Vertex-or-not unreadable; no chip = the ledger never saw it). The whole page is body-scroll (gdn-jigepa) so Safari offers Full Page screenshots. A repo with a live session is hidden from the launch list (gdn-wuvujo) — it's already in the roster, and a 2nd launch into it would collide on the shared JSONL. Pick a repo → two lane buttons (Vertex | Teams). The conversation page is single-session; leaving it (folder-lozenge tap, `/exit`, session end, or bare `/`) returns to the launcher. Flick = launcher ⇄ tap-in/tap-out.
