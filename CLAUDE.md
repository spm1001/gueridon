...
| GET | `/sessions` | **Launcher (gated):** the launcher roster — EVERY live `claude` session (gdn-batogo), not just RC ones. `{sessions:[{pid,name,cwd,ageSec,kind,attachable,url,ready}]}`, newest first. Four kinds (gdn-riheri, +gdn-kuhaku): `kind:"rc"` = Guéridon-spawned RC (attachable; Open = claude.ai URL, End = `DELETE /launch`); `kind:"vertex"` = Guéridon's own streaming-lane `-p` session (attachable; Open = RAW `/#folder` — never percent-encoded, End = `POST /exit`); `kind:"vertex-terminal"` = a Vertex-billed session Guéridon did NOT launch (a `claudev`/`claudefv` terminal session — detected from `/proc` environ OR the cmdline `--settings` blob; shown "vertex · terminal"); `kind:"local"` = hand-started/foreign non-Vertex (a Claude Desktop-app agent, a terminal session in `~`, or an RC session orphaned by a bridge restart). Both foreign kinds are **End-able but NOT attachable** (End = `DELETE /session/:pid`, gdn-racuca — SIGTERM by pid, graceful/resumable, no pty needed; no Open — attach/takeover still parked as gdn-kidowe). `/proc`-scans `comm=="claude"`, cross-refs `rcSessions` + the `-p` `sessions` map by pid; own-vertex wins over the generic vertex-billed detection. **The CC daemon (`claude daemon run …`) is excluded** (gdn-mimije — it shares `comm=="claude"` but is infra, not a session; `isDaemonCmdline` in `sessions.ts` skips `argv[1]==="daemon"`). **KNOWN LEAK, same family (gdn-caguga, verified live 2026-07-25):** a `claude remote-control` **server** is NOT excluded — tube's `claude-remote@home-modha-notes` systemd service appears as `kind:"local" name:"~/notes"`, End-able. Harm is not just cosmetic: since gdn-wuvujo hides repos that have a live session, and a server always looks like one, **a `claude-remote@` instance on any repo under `SCAN_ROOT` permanently hides that repo from the launch list.** Its session children are separately identifiable by `CLAUDE_CODE_ENVIRONMENT_KIND=bridge` + `ENTRYPOINT=sdk-cli` + `CHILD_SESSION=1` in `/proc` environ |
