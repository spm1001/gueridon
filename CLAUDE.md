...
| GET | `/sessions` | **Launcher (gated):** the launcher roster — EVERY live `claude` session (gdn-batogo), not just RC ones. `{sessions:[{pid,name,cwd,ageSec,kind,attachable,url,ready}]}`, newest first. Four kinds (gdn-riheri, +gdn-kuhaku): `kind:"rc"` = Guéridon-spawned RC (attachable; Open = claude.ai URL, End = `DELETE /launch`); `kind:"vertex"` = Guéridon's own streaming-lane `-p` session (attachable; Open = RAW `/#folder` — never percent-encoded, End = `POST /exit`); `kind:"vertex-terminal"` = a Vertex-billed session Guéridon did NOT launch (a `claudev`/`claudefv` terminal session — detected from `/proc` environ OR the cmdline `--settings` blob; shown "vertex · terminal"); `kind:"local"` = hand-started/foreign non-Vertex (a Claude Desktop-app agent, a terminal session in `~`, or an RC session orphaned by a bridge restart). Both foreign kinds are **End-able but NOT attachable** (End = `DELETE /session/:pid`, gdn-racuca — SIGTERM by pid, graceful/resumable, no pty needed; no Open — attach/takeover still parked as gdn-kidowe). `/proc`-scans `comm=="claude"`, cross-refs `rcSessions` + the `-p` `sessions` map by pid; own-vertex wins over the generic vertex-billed detection. **Claude INFRA processes are excluded** (`isInfraCmdline` in `sessions.ts`, matching subcommand `argv[1]` ∈ {`daemon`, `remote-control`, `remote`}): the CC daemon (gdn-mimije) and `claude remote-control` **servers** (gdn-caguga — a `claude-remote@` systemd unit used to appear as an End-able `kind:"local"` row, and because gdn-wuvujo hides repos with a live session, a server on a `SCAN_ROOT` repo permanently hid that repo from the launch list; fixed 2026-07-26). The Teams lane's `--remote-control` FLAG form (dashes) is a real session and is NOT excluded. A server's session **children** deliberately stay in the roster (they're live work; identifiable by `CLAUDE_CODE_ENVIRONMENT_KIND=bridge` + `ENTRYPOINT=sdk-cli` + `CHILD_SESSION=1` in `/proc` environ; they classify as `local` — no 5th kind while gdn-sudacu has the Teams lane under review) |
