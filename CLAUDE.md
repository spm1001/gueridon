...
| GET | `/recent` | **Launcher (gated like `/sessions`):** the uuid-keyed layer (gdn-vucube) — recent COLD sessions from the transcript farm, cold-by-construction (excludes every uuid the liveness union knows: /proc scan sessionUuids + Guéridon's own registries + per-wallet `claude agents --json`). `{sessions:[{uuid,name,cwd,ageSec,title,titleSource,entrypoint,uuidVersion}]}` — titles scavenged ai-title → Cowork sidecar → RC bridge-log scrape → first prompt (`server/session-index.ts`, deglacer-schema-informed). 5-day window, cap 15, 20s response cache. Read-only v1 — pickup verbs land with gdn-merozu |
